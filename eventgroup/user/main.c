...
void task1(void *pvParameters)
{
	...
	while(1)
	{
		...
		{
			Debug("\n\rtask1 set the EVENTBIT_1.");
			xEventGroupSetBits(EventGroupHandler,EVENTBIT_1);
		}
		if(task1_runtime % 2 == 0)
		{
			Debug("\n\rtask1 set the EVENTBIT_0.");
...
void task2(void *pvParameters)
{
	...
	while(1)
	{
		...
		Debug("\n\rtask2 get the event: %d.",EventValue);
		task2_runtime++;	
...
void task3(void *pvParameters)
{
	...
	while(1)
	{
		...
		task3_runtime++;
		Debug("\n\rtask3 wait the event.");
		EventValue=xEventGroupWaitBits((EventGroupHandle_t	)EventGroupHandler,		
									   (EventBits_t			)EVENTBIT_0,
									   (BaseType_t			)pdTRUE,				
									   (BaseType_t			)pdTRUE,
									   (TickType_t			)portMAX_DELAY);
		Debug("\n\rtask3 get the event: %d.",EventValue);
