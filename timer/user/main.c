...
SemaphoreHandle_t AutoReloadBinarySemaphore;
SemaphoreHandle_t OneShotBinarySemaphore;
...
{
	AutoReloadBinarySemaphore=xSemaphoreCreateBinary();
	OneShotBinarySemaphore=xSemaphoreCreateBinary();
...
void task1(void *pvParameters)
{
	...
	while(1)
	{
		...
		err=xSemaphoreTake(AutoReloadBinarySemaphore,portMAX_DELAY);
		if(err==pdTRUE)
		{
			Debug("\n\rtask1 get the ARsemaphore.");
		}
...
void task2(void *pvParameters)
{
	...
	while(1)
	{
		...
		err=xSemaphoreTake(OneShotBinarySemaphore,4000);
		if(err==pdTRUE)
		{
			Debug("\n\rtask2 get the OSsemaphore.");
		}
		else
		{
			Debug("\n\rtask2 failed get the OSsemaphore.");
		}
...
void AutoReloadCallback(TimerHandle_t xTimer)
{
	xSemaphoreGive(AutoReloadBinarySemaphore);
	Debug("\n\rgive auto reload sema");
...
void OneShotCallback(TimerHandle_t xTimer)
{
	xSemaphoreGive(OneShotBinarySemaphore);
	Debug("\n\rgive one shot sema");
