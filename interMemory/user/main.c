...
void task1(void *pvParameters)
{
	...
	while(1)
	{
		...
		if(buffer!=NULL)
		{
			Debug("\n\rfree buffer, the buffer context is: %s",buffer);
			vPortFree(buffer);
...
task1sleep:		
		...
		if(After == 0 && Before == 0 )
		{
			applybefore = xPortGetFreeHeapSize();
			count = sys_count * 10+6;
			buffer=pvPortMalloc(count);
			freemem=xPortGetFreeHeapSize();
			Debug("\n\rtask1 apply the buffer addr is: %#x, apply %d - %d = %d",(int)buffer,applybefore,freemem,count);
			
			strncpy(buffer,task1name,strlen(task1name));
			vTaskDelay(5000);
		}
		else
		{
...
void iwdg_task(void *pvParameters)
{
	...
	rtc_set_wakeup_time.RTC_Seconds = 2;
	...
	{		
		Debug("\n\rfeed dog ");
		IWDG_Feed();
		iwdgname = pcTaskGetName(IWDG_Handle);
		if(buffer!=NULL)
		{
			Debug("\n\riwdg free buffer, context is: %s",buffer);
			vPortFree(buffer);
		...
		buffer=pvPortMalloc(count);
		freemem=xPortGetFreeHeapSize();
		Debug("\n\riwdg apply the addr is: %#x,apply  %d - %d = %d",(int)buffer,applybefore,freemem,count);
		strncpy(buffer,iwdgname,strlen(iwdgname));
	}
}
